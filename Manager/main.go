...
func initConfig(config parseConfig.Config){
    ...
    Consumptionratio = float64(config.Get("Consumptionratio").(float64))
}
...
func makeUpdateQueue(umymap map[string]map[string]string){
    if(len(umymap) > 0){
        ...
        for k, v := range umymap{
            uuu, err := strconv.ParseFloat(v["u"],64)
            if err != nil {
                fmt.Printf("uuu: %v\n",uuu)
            }
            ddd, err := strconv.ParseFloat(v["d"],64)
            if err != nil {
                fmt.Printf("ddd: %v\n",ddd)
            }
            ...
            ids = ids + k + ","
            us = us + "WHEN " + v["id"] + " THEN u + " + strconv.FormatFloat(uuu,'f',0,64) + "\n"
            ds = ds + "WHEN " + v["id"] + " THEN d + " + strconv.FormatFloat(ddd,'f',0,64) + "\n"
